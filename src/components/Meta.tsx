...
export function Meta({ title, description, imageUrl, timestamp }: MetaProps) {
  const formattedTitle = `Taralend - ${title}`;
  return (
    <Head>
      <title>{formattedTitle}</title>{' '}
      <meta name="description" content={description} key="description" />
      <meta property="og:title" content={`Taralend - ${title}`} key="title" />
      ...
      {imageUrl && (
        <meta name="twitter:image:alt" content={`taralend governance image`} key="twitteralt" />
      )}
      <meta name="twitter:site" content="@taralend_xyz" key="twittersite" />
      <meta
      ...
      />
      <link rel="apple-touch-icon" href="/taralend_180.png" />
      <meta name="apple-mobile-web-app-title" content={`Taralend`} />
